  ...
  try {
    ...
    if (data && data.length > 0) {
      const items = data.map(item => ({
        ...
        cocina: (item.cocina ?? 'cocina_a') as Kitchen,
...
export async function cargarOrdenes(setState: SetState, tenantId?: string) {
  try {
    ...
    const orders: Order[] = data.map(row => ({
      ...
      claimedByKitchen: row.claimed_by_kitchen,
    ...
    const tableSessions: TableSession[] = sessions.map(row => ({
      ...
      orders: (ordersData ?? [])
        ...
        .map(o => ({
          ...
          cocinaAStatus: (o.cocina_a_status ?? 'en_cola') as KitchenStatus,
          cocinaBStatus: (o.cocina_b_status ?? 'en_cola') as KitchenStatus,
          createdAt: new Date(o.created_at),
          updatedAt: new Date(o.updated_at),
          nombreCliente: o.nombre_cliente,
          telefono: o.telefono,
          claimedByKitchen: o.claimed_by_kitchen,
