  ...
  return (
    <div className="p-4 space-y-5 max-w-3xl">
      <div>
        ...
        <p className="text-xs text-[#6B6B6B] mt-0.5">{feedbackList.length} respuestas recibidas</p>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-3 gap-3">
        <div className="border border-[#E5E5E5] rounded-xl p-4 bg-white text-center">
          <Star className="h-5 w-5 mx-auto mb-1 text-yellow-500 fill-yellow-500" />
          <p className="text-2xl font-bold text-black">{avgRating}</p>
          <p className="text-[10px] text-[#6B6B6B] uppercase tracking-wide">Promedio</p>
        </div>
        <div className="border border-[#E5E5E5] rounded-xl p-4 bg-white text-center">
          <TrendingUp className="h-5 w-5 mx-auto mb-1 text-[#16A34A]" />
          <p className="text-2xl font-bold text-black">{nps}%</p>
          <p className="text-[10px] text-[#6B6B6B] uppercase tracking-wide">Satisfechos (≥4★)</p>
        </div>
        <div className="border border-[#E5E5E5] rounded-xl p-4 bg-white text-center">
          <MessageSquare className="h-5 w-5 mx-auto mb-1 text-[#6B6B6B]" />
          <p className="text-2xl font-bold text-black">{feedbackList.filter(f => f.comentario).length}</p>
          <p className="text-[10px] text-[#6B6B6B] uppercase tracking-wide">Con comentario</p>
        </div>
      </div>
        ...
        <div className="p-4 space-y-2">
          {distribution.map(d => (
            <div key={d.star} className="flex items-center gap-3">
              ...
              <span className="text-xs text-[#6B6B6B] w-8 text-right">{d.count}</span>
            </div>
          ))}
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-0.5">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        ...
                        className={cn(
                          'h-3 w-3',
                          i < f.rating ? 'fill-yellow-400 text-yellow-400' : 'text-[#E5E5E5]'
                        )}
                      />
                    ))}
                  </div>
